package com.rogo.dicodingcapstone.views.home
...
import com.rogo.dicodingcapstone.databinding.FragmentHomeBinding
...
class HomeFragment : Fragment() {
    ...
    private var nowAdapter: WeakReference<MovieAdapter>? = null
    private var popularAdapter: WeakReference<MovieAdapter>? = null
    private var topAdapter: WeakReference<MovieAdapter>? = null
        ...
        viewModel.getMovieNow.observe(viewLifecycleOwner) {
            when (it) {
                ...
                is Resource.Success -> {
                    ...
                    // CHANGE: Safely access adapter
                    nowAdapter?.get()?.differ?.submitList(it.data)
                }
        ...
        viewModel.getMoviePopular.observe(viewLifecycleOwner) {
            when (it) {
                ...
                is Resource.Success -> {
                    ...
                    // CHANGE: Safely access adapter
                    popularAdapter?.get()?.differ?.submitList(it.data)
                }
        ...
        viewModel.getMovieTopRated.observe(viewLifecycleOwner) {
            when (it) {
                ...
                is Resource.Success -> {
                    ...
                    // CHANGE: Safely access adapter
                    topAdapter?.get()?.differ?.submitList(it.data)
                }
    ...
    private fun initAdapter() {
        // CHANGE: Use WeakReferences for adapters
        nowAdapter = WeakReference(MovieAdapter {
            findNavController().navigate(
                R.id.action_homeFragment_to_detailFragment,
                Bundle().apply { putParcelable(DetailFragment.EXTRA_MOVIE, it) })
        })
        popularAdapter = WeakReference(MovieAdapter {
            findNavController().navigate(
                R.id.action_homeFragment_to_detailFragment,
                Bundle().apply { putParcelable(DetailFragment.EXTRA_MOVIE, it) })
        })
        topAdapter = WeakReference(MovieAdapter {
            findNavController().navigate(
                R.id.action_homeFragment_to_detailFragment,
                Bundle().apply { putParcelable(DetailFragment.EXTRA_MOVIE, it) })
        })
        binding.rvMovieNowPlaying.apply {
            ...
            adapter = nowAdapter?.get()
        }
        binding.rvMoviePopular.apply {
            layoutManager =
                LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
            setHasFixedSize(true)
            adapter = popularAdapter?.get()
        }
        binding.rvMovieTopRated.apply {
            layoutManager =
                LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
            setHasFixedSize(true)
            adapter = topAdapter?.get()
        }
    }
